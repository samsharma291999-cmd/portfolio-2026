...
export default function ImpactHighlights() {
  return (
    <section className="border-t border-border-hairline px-5 py-20 sm:px-6 sm:py-24 lg:px-8">
        ...
        <div className="grid grid-cols-2 gap-x-8 gap-y-12 sm:grid-cols-4 lg:gap-x-12">
          {impactFigures.map((item, i) => (
            <motion.a
              ...
              transition={{ duration: 0.36, ease, delay: i * 0.06 }}
              className={`block ${item.externalLink || item.linkedCase ? "group" : ""}`}
            >
              <p className="font-[family-name:var(--font-display)] text-[clamp(1.75rem,3.5vw,2.75rem)] font-semibold leading-none text-ink whitespace-nowrap">
                {item.figure}
              </p>
